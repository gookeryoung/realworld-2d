...
void
hashnode_del(hashnode *n, del_func deleter) {
    deleter(n->data);
...
gint32
__hashtable_get_random_key(hashtable *t) {
...
gint32
hashtable_insert(hashtable *ht, void *data) {
    GK_ASSERT(ht);
    GK_ASSERT(data);
...
gbool
hashtable_remove(hashtable *ht, gint32 key, del_func deleter) {
    GK_ASSERT(ht);
